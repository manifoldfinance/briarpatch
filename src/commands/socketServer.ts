...
const getTokensInPairs = (
  ...
  for (const pairId of pairData.getPairIds()) {
    ...
    if (
      excludePairsWithTokens.length > 0 &&
...
const _preprocessRouteReq = (
  ...
  if (amount) {
    ...
    if (isNaN(amountValue) || amountValue < 0) {
      sanitizeStr +=
        '"amount" must be a string that parses to a number greater than 0.\n';
    }
  }
  ...
  if (options) {
    for (const property in _options) {
      if (options.hasOwnProperty(property)) {
        ...
        sanitizeStr += sanitizePropertyType(
          `options.${property}`,
          options[property],
        );
        ...
        if (propertyParams.type === 'int') {
          ...
          propertyParams.type === 'boolean' &&
          options.hasOwnProperty(property)
        ) {
          _options[property].value =
            options[property] === 'true' ? true : false;
          continue;
        }
        ...
        if (property === 'block') {
          // Special case for block; only check minimum.
          if (value <= propertyParams.min) {
            sanitizeStr +=
              `options.${property} must be parsable from a string to a ${propertyParams.type} ` +
              `greater than ${propertyParams.min}.\n`;
            continue;
          }
        } else if (
          !(
            property === 'max_hops' && // Special case for max hops--ignore it if specified:
...
const _processRouteReq = async (
  ...
  const _routesP: Promise<t.VFRoutes> = routeCache
    .getRoutes(source, dest, { maxHops: options.max_hops.value })
    .catch((error) => {
      // TODO: signal an error in routing to the client
      return [];
    });

  const _uniRouteP: Promise<any> = getUniRouteV2(source, dest, amount).catch(
    (error) => {
      return { error };
    },
  );
  ...
  _routeStats.uniRouteFound =
    _uniRoute && _uniRoute.routePath && _uniRoute.routePath.length > 0;
  if (!_routeStats.uniRouteFound) {
    _routeStats.uniError = _uniRouteResult.error
      ? _uniRouteResult.error
      : 'Unknown error.';
  }
      ...
      try {
        ...
        for (let idx = 0; idx < _uniRoute.routePath.length - 1; idx++) {
          ...
          if (edgeData && edgeData.hasOwnProperty('pairIds')) {
            ...
            if (edgeData.pairIds.length !== 1) {
              log.warn(
                `Building UNI route, edge data pairIds length is ${edgeData.pairIds.length} for ${src} to ${dst}.`,
              );
            }
          } else {
            throw Error(
              `Unable to build UNI route; no edge data or pairIds for edge ${src} to ${dst}`,
            );
          }
  ...
  const _quotedRoutes: t.VFRoutes = await quoteRoutes(
    uniData.pairData,
    uniData.tokenData,
    _routes,
    amount,
  ...
  _quotedRoutes.sort((a: t.VFRoute, b: t.VFRoute) => {
    // Sort descending by amount of destination token received
  ...
  const _requestedQuotedRoutes: t.VFRoutes = _quotedRoutes.slice(
    0,
    options.max_results.value,
  );
  if (uniData.wethPairData) {
    await rg.annotateRoutesWithUSD(
      uniData.pairData,
  ...
  return {
    routes: _requestedQuotedRoutes,
    uniRoute: _uniRoute.routeText,
    routeStats: _routeStats,
  };
};

const _processMultiPathRouteReq = async (
  _uniData: t.UniData,
  ...
  //
  const { routes, routeStats } = await _processRouteReq(
    _uniData,
    _routeCache,
    source,
    dest,
  ...
  const uniRouteArr = routes.filter((route: t.VFRoute) => {
    return route && route.length && route[0].isUni === true;
  });
  ...
  if (
    ...
    uniYield.token = lastUniSeg.dstAmount
      ? parseFloat(lastUniSeg.dstAmount)
      : 0.0;
  }
  // log.debug(`Routes:\n` +
  //           `================================================================================`)
  ...
  if (routes && routes.length && routes[0] && routes[0].length) {
    ...
    valveOnePathYield.token = lastSeg.dstAmount
      ? parseFloat(lastSeg.dstAmount)
      : 0.0;
  }
  ...
  const prunedRoutes = rg.pruneRoutes(routes, {
    maxRoutes: maximumConcurrentPaths,
    minGainToDest,
  });
  ...
  //
  const mpRouteTree: rt.TradeTreeNode | undefined =
    rt.buildTradeTree(prunedRoutes);
  if (mpRouteTree) {
  ...
  if (mpRouteTree) {
    ...
    if (_uniData.wethPairData) {
      await rt.annotateTradeTreeWithUSD(
        _uniData.pairData,
      ...
      (node, context) => {
        if (node.children.length === 0 && node.value.trades) {
          ...
          if (properties && properties.length) {
            ...
            valveMultiPathYield.usd += trade.outputUsd
              ? parseFloat(trade.outputUsd)
              : 0.0;
  ...
  const routeData = new RouteData(source, srcSymbol, dest, dstSymbol);
...
const _routeDataToPages = (routeData: RouteData): any => {
  ...
  const routeStr =
    `${routeData.getSourceSymbol()} (${routeData.getSourceAddr()}) --> ` +
  ...
  if (uniYield && spYield && spElements) {
    ...
    routeStatsStr +=
      routesFound !== undefined ? `${routesFound} routes found.` : '';
    routeStatsStr +=
      routesMeetingCriteria !== undefined
        ? ` ${routesMeetingCriteria} match criteria supplied.`
        : '';
  ...
  if (uniYield && mpYield && mpElements) {
    ...
    } = routeData.getRouteStats();
    let routeStatsStr: string = '';
    routeStatsStr +=
      routesFound !== undefined ? `${routesFound} routes found.` : '';
    routeStatsStr +=
      mpRoutesMeetingCriteria !== undefined
        ? ` ${mpRoutesMeetingCriteria} match criteria supplied.`
        : '';
    routeStatsStr +=
      mpRoutesAfterRmDuplicatePathPairs !== undefined
        ? ` ${mpRoutesAfterRmDuplicatePathPairs} after removing dup. lower order pairs.`
        : '';
    description.push({ text: routeStatsStr });
...
const _createReport = (
  ...
  const reportPage: any = {
    ...
    description: [
      { text: reportParameters.analysisDescription },
      {
  ...
  for (const tradeStat of tradeStats) {
    ...
    if (routeStats.vfiError === USDC_CONVERT_ERR) {
      ...
      row +=
        ', (No route found or unable to convert USD amount to tokens for trade.)';
      ...
      if (tradeStat.spDelta !== undefined) {
        const numTokensInStr = isNaN(tradeStat.inputAmount)
          ? ''
          : tradeStat.inputAmount.toFixed(6);
        const numTokensOutStr =
          tradeStat.spYield && tradeStat.spYield.token
  ...
  content.push({
    row: `${spBetter.length} - Single Path Routes Performed Better`,
    type: 'sub-section',
    collapsible: spBetter.length > 0,
  });
  spBetter.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${spWorse.length} - Single Path Routes Performed Lower`,
    type: 'sub-section',
    collapsible: spWorse.length > 0,
  });
  spWorse
    .slice()
    .reverse()
  ...
  content.push({
    row: `${spSame.length} - Single Path Routes Performed Similarly`,
    type: 'sub-section',
    collapsible: spSame.length > 0,
  });
  spSame.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${spIncomparable.length} - Single Path Routes Couldn't Be Compared`,
    type: 'sub-section',
    collapsible: spIncomparable.length > 0,
  });
  spIncomparable.forEach((row: any) => {
    content.push(row);
  });
  ...
  for (const tradeStat of tradeStats) {
    ...
    if (
      routeStats.vfiError === USDC_CONVERT_ERR ||
      ...
      if (tradeStat.mpDelta !== undefined) {
        const numTokensInStr = isNaN(tradeStat.inputAmount)
          ...
          : tradeStat.inputAmount.toFixed(6);
        const numTokensOutStr =
          tradeStat.mpYield && tradeStat.mpYield.token
        ...
        if (isNaN(mpDelta)) {
          ...
          if (mpDelta < 0) {
            row =
              `${tradeStat.mpDelta.toFixed(DELTA_PRECISION)}% worse, ` + row;
            mpWorse.push({ row, src, dst, view: true });
          } else {
            row =
              `+${tradeStat.mpDelta.toFixed(DELTA_PRECISION)}% better, ` + row;
  ...
  content.push({
    row: `${mpBetter.length} - Multi-Path Routes Performed Better`,
    type: 'sub-section',
    collapsible: mpBetter.length > 0,
  });
  mpBetter.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${mpWorse.length} - Multi-Path Routes Performed Lower`,
    type: 'sub-section',
    collapsible: mpWorse.length > 0,
  });
  mpWorse
    .slice()
    .reverse()
  ...
  content.push({
    row: `${mpSame.length} - Multi-Path Routes Performed Similarly`,
    type: 'sub-section',
    collapsible: mpSame.length > 0,
  });
  mpSame.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${mpIncomparable.length} - Multi-Path Routes Couldn't Be Compared`,
    type: 'sub-section',
    collapsible: mpIncomparable.length > 0,
  });
  mpIncomparable.forEach((row: any) => {
    content.push(row);
  });
  ...
  content.push({
    row: `${uniFail.length} - Routes Uniswap V2 Did Not Find`,
    type: 'sub-section',
    collapsible: uniFail.length > 0,
  });
  uniFail.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${spFail.length} - Routes Valve Finance Did Not Find`,
    type: 'sub-section',
    collapsible: spFail.length > 0,
  });
  spFail.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${spCriteriaFail.length} - Routes Did Not Match Specified Criteria`,
    type: 'sub-section',
    collapsible: spCriteriaFail.length > 0,
  });
  spCriteriaFail.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${mpCriteriaFail.length} - Routes Did Not Match Specified Multi-Path Criteria`,
    type: 'sub-section',
    collapsible: mpCriteriaFail.length > 0,
  });
  mpCriteriaFail.forEach((row: any) => {
    content.push(row);
  });

  content.push({
    row: `${spUsdcConvFail.length} - Routes Couldn't Be Evaluated From A $USD Initial Amount`,
    type: 'sub-section',
    collapsible: spUsdcConvFail.length > 0,
  });
  spUsdcConvFail.forEach((row: any) => {
    content.push(row);
  });
...
export const startSocketServer = async (port: string): Promise<void> => {
  log.info(
    `Starting Uniswap Routing Socket Server on port ${port}...\n` +
  ...
  socketServer.on('connection', (socket: socketio.Socket) => {
    ...
    log.debug(
      `${socket.id} connected (${
        Object.keys(clientSockets).length
      } connections).`,
    );

    socket.on(
      'route',
      async (source: string, dest: string, amount: string, options?: any) => {
        ...
        if (_error !== '') {
          ...
          const routeReqObj = await _processRouteReq(
            _uniDataQuotable,
            _routeCache,
            source,
            dest,
          ...
          const legacyFmtRoutes = rg.convertRoutesToLegacyFmt(
            _uniDataQuotable.pairData,
            _uniDataQuotable.tokenData,
            routeReqObj.routes,
          );
          socket.emit(reqType, {
            requestId,
    ...
    socket.on('usdTokenQuote', (source: string, usdAmount: string) => {
      ...
      if (!isNaN(parseFloat(usdAmount)) && _uniDataQuotable.wethPairData) {
        tokens = getEstimatedTokensFromUSD(
          _uniDataQuotable.pairData,
    ...
    socket.on(
      ...
      async (source: string, dest: string, amount: string, options?: any) => {
        ...
        const routeData = await _processMultiPathRouteReq(
          _uniDataQuotable,
          _routeCache,
          source,
          dest,
    ...
    socket.on('report-generate', async (reportParameters: any) => {
      ...
      if (fs.existsSync(reportFilePath)) {
        try {
          ...
          log.debug(
            `Returned existing report ${reportFilePath}. Skipping generation of new report.`,
          );
          return;
        } catch (error) {
          log.warn(
            `Failed to find/read ${reportFilePath}. Generating new report.`,
          );
        }
      }
      path.pop();
      ...
      fs.writeFileSync(
        paramsFilePath,
        JSON.stringify(reportParameters, null, 2),
      );
      path.pop();
      ...
      switch (tokenSet) {
        ...
        default:
          c.uniswapDefaultTokens.forEach((tokenObj) =>
            _tokenSet.add(tokenObj.address.toLowerCase()),
          );
          break;
      }
      ...
      for (const src of _tokenSet) {
        if (!c.tokensNoWethPair.includes(src)) {
          // Ignore these tokens as source since we can't
      ...
      trades.sort((tradeA: Trade, tradeB: Trade) => {
        return tradeB.randomValue - tradeA.randomValue;
      }); // Asc. order.
      ...
      while (tradeCount < numTrades) {
        ...
        const amountSrc = getEstimatedTokensFromUSD(
          _uniData.pairData,
          _uniData.wethPairData,
          src,
          tradeAmount,
        ...
        if (amountSrc === '') {
          ...
          const failRouteData = new RouteData(
            src,
            srcSymbol,
            dst,
            dstSymbol,
        ...
        log.debug(
          `Processing trade ${tradeCount} of ${numTrades} (${srcSymbol} --> ${dstSymbol}, ` +
            `${amountSrc}tokens, $${tradeAmount}USD).`,
        );

        const routeData: RouteData = await _processMultiPathRouteReq(
          _uniData,
          _routeCache,
          src,
          dst,
        ...
        fs.writeFile(filePath, routeData.serialize(), (err) => {
          err && log.warn(`Failed to write ${filePath} because\n${err}`);
        });
        path.pop();
      ...
      //
      const reportPage = _createReport(
        reportParameters,
        paramsHash,
        tradeStats,
      );
      ...
      fs.writeFile(reportFilePath, JSON.stringify(reportPage), (err) => {
        err && log.warn(`Failed to write ${reportFilePath} because\n${err}`);
      });
      ...
      for (const socketId in clientSockets) {
        ...
        log.debug(
          `Emitting 'report-update' to ${socketId} (${
            clientSocket ? 'socket exists' : 'socket undefined'
          })`,
        );
    ...
    socket.on('report-fetch-route', async (routeParameters: any) => {
      ...
      if (!paramsHash || !src || !dst) {
        socket.emit('report-fetch-route', {
          error: 'Invalid request--paramsHash, src and dst must be defined.',
        });
      } else {
        const REPORTS_DIR = 'reports';
        const routeFileName = `${src}_${dst}.json`;
        const path = [REPORTS_DIR, paramsHash, routeFileName];
        ...
        fs.readFile(path.join('/'), (err, data) => {
          if (err) {
            socket.emit('report-fetch-route', {
              error: `Failed reading route information.\n${err}`,
            });
          } else {
            ...
            log.debug(
              `report-fetch-route: route pages length ${pages.length}.`,
            );
