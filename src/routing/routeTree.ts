...
export interface TradeTreeNode {
  value: {
    ...
    // }
    gainToDest?: {
      // The gain to destination, (1-slippageN)*(1-slippageM) ..., for
    ...
    // }
    trades?: {
      // The trade object for a particular proportion of the trade amount
...
const _pruneTreeRoute = (
  node: TradeTreeNode,
  routeId: string,
  nodesToDelete: TradeTreeNode[],
): void => {
...
export const buildTradeTree = (
  routes: t.VFRoutes,
): TradeTreeNode | undefined => {
    ...
    for (const seg of route) {
      ...
      if (!node) {
        ...
        if (node.value.address !== seg.src) {
          throw Error(
            `buildTradeTree: root node does not match address ` +
      ...
      //
      let dstNode: TradeTreeNode | undefined = node.children.find(
        (searchNode: TradeTreeNode) => searchNode.value.address === seg.dst,
      );
      if (!dstNode) {
        dstNode = {
          value: {
      ...
      if (dstNode.value.gainToDest !== undefined) {
        dstNode.value.gainToDest[routeIdx] =
          seg.gainToDest === undefined ? 0.0 : seg.gainToDest;
      }
      node = dstNode;
    }
  }
...
const _getTradeProportions = (startNode: TradeTreeNode): TradeProportions => {
  ...
  if (startNode.children.length === 1) {
    ...
    tradeProportions.forEach((value: TradeProportion, index: number) => {
      if (
        value.maxGainToDest > 1.0 &&
        value.maxGainToDest > maxGainBeyondUnity
      ) {
    ...
    if (unityGainExceedIndex >= 0) {
      ...
      log.warn(
        `Found trade with gain exceeding unity (${tradeProp.maxGainToDest}).\n`,
      );
...
//    2. Update pair data
//
export const costTradeTree = async (
  allPairData: t.Pairs,
  tokenData: t.Tokens,
  amount: string,
  rootNode: TradeTreeNode,
  ...
  for (const level in bfsVisitSchedule) {
    ...
    if (level === '1') {
      ...
      if (nodesAtLevel.length !== 1) {
        throw Error(
          `costTradeTree: expected one node at level 1 (root), found ${nodesAtLevel.length}.`,
        );
      }
      ...
      for (const tradeProp of tradeProportions) {
        ...
        const proportion =
          tradeProp.proportion === undefined ? 0 : tradeProp.proportion;
        ...
        if (
          ...
          inputAmount =
            proportion *
            parseFloat(childNode.parent.value.trades[tradeId].outputAmount);
        } else {
          throw Error(
            `costTradeTree: expected parent tree node to have trades with tradeId ${tradeId}\n` +
              `parent:\n${JSON.stringify(
                childNode.parent ? childNode.parent.value : undefined,
                null,
                2,
              )}`,
          );
        ...
        if (!pairId) {
          throw Error(
            `costTradeTree: pair not specified on tree node.\n` +
        ...
        try {
          estimate = computeTradeEstimates(
            pairData,
            tokenData,
            childNode.parent.value.address,
            inputAmount.toString(),
          );
        } catch (estimateError) {
          throw new Error(
            `costTradeTree: failed cost estimate.\n` +
...
export const annotateTradeTreeWithUSD = async (
  allPairData: t.Pairs,
      ...
      if (node.value.trades) {
        for (const tradeId in node.value.trades) {
          ...
          if (node.parent) {
            ...
            const input = trade.inputAmountP
              ? trade.inputAmountP
              : trade.inputAmount;
            trade.inputUsd = getEstimatedUSD(
              allPairData,
              wethPairDict,
          ...
          trade.outputUsd = getEstimatedUSD(
            allPairData,
            wethPairDict,
            node.value.address,
            trade.outputAmount,
...
export const pruneRoutesWithDuplicatePairs = (rootNode: TradeTreeNode) => {
  ...
  for (const pairId in pairNodeInfoMap) {
    ...
    for (
      let pairInfoIdx = 0;
      pairInfoIdx < pairNodeInfo.length - 1;
      pairInfoIdx++
    ) {
  ...
  crawl(
    ...
    (node, context) => {
      ...
      if (maxRouteGTD > HIGH_MGTD) {
        log.debug(
          `Route ${maxRouteId} at level ${context.level} exceeds high MGTD (gtd=${maxRouteGTD}).`,
        );
