...
export class RouteCache {
  constructor(pairGraph: t.PairGraph, constraints?: t.Constraints) {
    this._routeCache = {
      oldestRouteMs: 0,
      newestRouteMs: 0,
  ...
  public async getRoutes(
    ...
    const _options: RouteOption = {
      forceUpdate:
        options.forceUpdate !== undefined ? options.forceUpdate : false,
    ...
    //                cache:
    if (
      this._routeConstraints.maxDistance &&
      this._routeConstraints.maxDistance < _options.maxHops
    ) {
      log.warn(
        `RouteCache::getRoutes:  Bypassing cache for maxhops specified ${_options.maxHops}`,
      );
      const constraints = {
        ...this._routeConstraints,
        ...{ maxDistance: _options.maxHops },
      };
      const stackedRoutes: t.VFStackedRoutes = r.findRoutes(
        this._pairGraph,
        srcId,
        dstId,
        constraints,
    ...
    let routesEntry: RouteCacheEntry | undefined =
      this._routeCache.cacheMap.get(key);
