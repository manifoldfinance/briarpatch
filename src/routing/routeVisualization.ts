 ...
 * @returns
 */
export const getCytoscapeGraph = (
  tradeTree: TradeTreeNode,
  useUuid = true,
): cytoscape.Core => {
  let _cyEleId = 0;
  let uuidLookup: any = {};
    ...
    (node, context) => {
      if (node) {
        ...
        let nodeColor = node.value.isUniRoute
          ? PATH_COLORS.UNI_NODE
          : PATH_COLORS.DEFAULT_NODE;
        let nodeShape = node.value.isBest
          ? NODE_SHAPES.VALVE_SP
          : NODE_SHAPES.DEFAULT;
        if (isMultiPath) {
        ...
        if (context.parent) {
          ...
          const impact = parseFloat(
            node.value.impact ? node.value.impact : '0',
          ).toFixed(3);
          // const label = `$${node.value.amountUSD},  ${impact}%`
          const label = `${impact}%`;
          const color =
            parent.value.isUniRoute && node.value.isUniRoute && !isMultiPath
              ? PATH_COLORS.UNI_EDGE
              : PATH_COLORS.DEFAULT_EDGE;
...
export const elementDataFromCytoscape = (cyGraph: cytoscape.Core): any => {
  ...
  const eleDatas = elements.map((ele: any) => {
