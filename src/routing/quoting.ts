...
export const getEstimatedUSD = (
  ...
  if (tokenId !== WETH_ADDR) {
    ...
    wethPerToken =
      wethPair.token0.id === WETH_ADDR
        ? wethPair.token0Price
        : wethPair.token1Price;
  }

  const usdcWethPairId: string = wethPairDict[USDC_ADDR];
  const usdcWethPair: t.Pair = allPairData.getPair(usdcWethPairId);
  const usdcPerWeth: string =
    usdcWethPair.token0.id === USDC_ADDR
      ? usdcWethPair.token0Price
      : usdcWethPair.token1Price;

  try {
    const amountUSD: number =
      parseFloat(tokenAmount) *
      parseFloat(wethPerToken) *
      parseFloat(usdcPerWeth);
 ...
 * }
 */
export const getEstimatedTokensFromUSD = (
  allPairData: t.Pairs,
  wethPairDict: t.WethPairIdDict,
  tokenId: string,
  usdAmount: string,
  ...
  try {
    const tokenAmount: number =
      ...
      parseFloat(tokenPerWeth) *
      parseFloat(wethPerUsdc);
    // log.debug(`getEstimateUSD (${tokenId}):\n` +
 ...
 *          input.
 */
export const computeTradeEstimates = (
  pairData: t.Pair,
  tokenData: t.Tokens,
  srcAddrLC: string,
  amount: string,
  ...
  if (!token0) {
    throw new Error(
      `Unable to find token data for token id ${pairData.token0.id}.`,
    );
  }
  if (!token1) {
    throw new Error(
      `Unable to find token data for token id ${pairData.token1.id}.`,
    );
  }
  ...
  const _pair = new Pair(
    new TokenAmount(token0UniObj, reserve0IntStr),
  ...
  const _tradeAmount = new TokenAmount(
    _srcToken.obj,
    getIntegerString(amount, _srcToken.decimals),
  );
...
export const getRoutePairIdsOfAge = (
  allPairData: t.Pairs,
  routes: t.VFRoutes,
  ageMs: number = 1 * avgBlockMs,
): Set<string> => {
  const now = Date.now();
  ...
  for (const route of routes) {
    for (const segment of route) {
      ...
      if (pairData && pairData.updatedMs && now - pairData.updatedMs < ageMs) {
        continue;
      }
...
export const getRoutePairIdsNotBlockNum = (
  allPairData: t.Pairs,
  blockNum: number,
  routes: t.VFRoutes,
): Set<string> => {
...
export const filterToPairIdsOfAge = (
  allPairData: t.Pairs,
  pairIds: Set<string>,
  ageMs: number = 1 * avgBlockMs,
): Set<string> => {
...
export const filterToPairIdsNotBlockNum = (
  allPairData: t.Pairs,
  pairIds: Set<string>,
  blockNum: number,
): Set<string> => {
...
export const quoteRoutes = async (
  ...
  if (blockNumber !== NO_BLOCK_NUM) {
    ...
    const pairIdsToUpdate: Set<string> = getRoutePairIdsNotBlockNum(
      allPairData,
      blockNumber,
      routes,
    );
     ...
     */
    const start: number = Date.now();
    const pairIdsToUpdate: Set<string> = getRoutePairIdsOfAge(
      allPairData,
      routes,
    );
    const updatedPairs: t.PairLite[] = await getUpdatedPairData(
    ...
    log.debug(
      `quoteRoutes: Finished updating ${pairIdsToUpdate.size} pairs in ${
        Date.now() - start
      } ms`,
    );
  }
  ...
  for (const route of routes) {
    ...
    for (const segment of route) {
      ...
      if (
        parseFloat(estimate.trade.priceImpact.toSignificant(18)) > maxImpact
      ) {
        exceededImpact = true;
        break;
      }
