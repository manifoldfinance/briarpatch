...
export class TradeStats {
  constructor(routeData: RouteData) {
    ...
    if (uniYield && spYield) {
      this.spDelta =
        uniYield.token > 0
          ? (100 * (spYield.token - uniYield.token)) / uniYield.token
          : NaN;
    }
    if (uniYield && mpYield) {
      this.mpDelta =
        uniYield.token > 0
          ? (100 * (mpYield.token - uniYield.token)) / uniYield.token
          : NaN;
    }
  }
...
export class RouteData {
  ...
  public getPercentDifferenceSinglePath(): number {
    ...
    if (uniYield && singlePathValveYield && uniYield.token > 0) {
      return (
        (100 * (singlePathValveYield.token - uniYield.token)) / uniYield.token
      );
    }
  ...
  public getPercentDifferenceMultiPath(): number {
    ...
    if (uniYield && multiPathValveYield && uniYield.token > 0) {
      return (
        (100 * (multiPathValveYield.token - uniYield.token)) / uniYield.token
      );
    }
