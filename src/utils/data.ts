...
export const initUniData = async (
  force = false,
  buildWethPairDict = true,
): Promise<t.UniData> => {
  log.info('Initializing SushiSwap data...');

  const ignoreExpiry =
    process.env.NODE_ENV !== 'production' &&
    configJson.ignore_expired_graph_data;
  const _rawPairData: any = await getPairData({
    ignorePersisted: force,
    ignoreExpiry,
  });
  const _allTokenData: any = await getTokenData({
    ignorePersisted: force,
    ignoreExpiry,
  });
...
const getPairData = async (options?: any): Promise<t.Pairs> => {
  ...
  try {
    ...
    log.warn(
      `❌ Unable to retrieve stored swap data. Fetching from Graph Protocol.`,
    );
  }
  ...
  if (_storedObj && _storedObj.hasOwnProperty('timeMs')) {
    const _storedInterval = Interval.fromDateTimes(
      DateTime.fromMillis(_storedObj.timeMs),
      DateTime.now(),
    );
    _storedAgeLimitExceeded =
      _storedInterval.length() > MAX_DATA_AGE.toMillis();
  }

  if (!_allPairs || (!_options.ignoreExpiry && _storedAgeLimitExceeded)) {
  ...
  if (_options.ignoreDeadTokens) {
    ...
    for (const _pairId of _allPairs.getPairIds()) {
      ...
      if (
        _deadTokensObj.object.hasOwnProperty(_pair.token0.id) ||
      ...
      if (
        bogusTokens.hasOwnProperty(_pair.token0.id) ||
    ...
    const removedPairs: number =
      _allPairs.getPairIds().length - _livePairs.getPairIds().length;
    log.info(
      `Ignore pairs with dead & bogus tokens removed ${removedPairs} pairs`,
    );
...
const getTokenData = async (options?: any): Promise<t.Tokens> => {
  ...
  try {
    ...
    log.warn(
      `❌ Unable to retrieve stored token data. Fetching from Graph Protocol.`,
    );
  }
  ...
  if (!_allTokens || (!_options.ignoreExpiry && _storedAgeLimitExceeded)) {
  ...
  if (_options.ignoreDeadTokens) {
    ...
    const removedTokens: number =
      _allTokens.getTokenIds().length - _liveTokens.getTokenIds().length;
...
const constructPairGraph = async (
  allPairData: t.Pairs,
): Promise<t.PairGraph> => {
  const _g: t.PairGraph = new graphlib.Graph({
    directed: false,
