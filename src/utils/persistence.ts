...
export const storeObject = async (
  ...
  try {
    ...
    _objectStr = formatted
      ? JSON.stringify(_object, null, 2)
...
export const retrieveObject = async (key: string): Promise<any> => {
  ...
  try {
    ...
    throw new Error(
      `❌ Failed to retrieve stringified object at key ${key}\n${error}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `❌ Unable to parse stringified object retrieved at key ${key}.\n` +
  ...
  if (!_object.hasOwnProperty('object') || !_object.hasOwnProperty('timeMs')) {
    throw new Error(
      '❌ Retrieved object appears to be corrupt and is missing one or ' +
